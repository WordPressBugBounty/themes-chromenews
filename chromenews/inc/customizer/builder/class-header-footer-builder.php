...
class Header_Footer_Builder
{
  ...
  public function athfb_register_customizer_controls($wp_customize)
  {
    ...
    $wp_customize->add_section(
      'header_builder',
      array(
        'title' =>  esc_html__('Header Builder', 'chromenews'),
        'priority' => 20,
      )
    );
    ...
    $wp_customize->add_section(
      'footer_builder',
      array(
        'title' =>  esc_html__('Footer Builder', 'chromenews'),
        'priority' => 25,
      )
    );
    ...
    $loadHeader = chromenews_get_option('athfb_show_checkbox_header');
    //if ($loadHeader) {
    ...
    $loadFooter = chromenews_get_option('athfb_show_checkbox_footer');

    $wp_customize->add_control(new Header_Footer_Builder_Control(
      $wp_customize,
      'footer_builder_data',
      array(
        'label' =>  esc_html__('Footer Layout', 'chromenews'),
    ...
    foreach (['header', 'footer'] as $builder_type) {
      ...
      if ($builder_type === 'header') {
        $message = esc_html__('Create dynamic header effortlessly with drag-and-drop builder.', 'chromenews');
        $label = esc_html__('Header', 'chromenews');
      } elseif ($builder_type === 'footer') {
        $message = esc_html__('Create dynamic footer effortlessly with drag-and-drop builder.', 'chromenews');
        $label = esc_html__('Footer', 'chromenews');
      }
      ...
      $wp_customize->add_control($checkbox_setting_id, [
        ...
        'label' =>  esc_html(sprintf(
          __("Enable %s Builder", 'chromenews'),
          $label
        )),
        'description' => $message,
        'priority' => 2,
      ...
      $wp_customize->add_control(new Element_Settings_Container_Control(
        ...
        [
          ...
          'transport' => 'postMessage',
          'priority' => 2,
          'active_callback' => "chromenews_is_active_builder_{$builder_type}"
        ]
      ));
    }
  ...
  public function athfb_enqueue_customizer_scripts()
  {
    ...
    wp_enqueue_script(
      ...
      get_template_directory_uri() . '/admin-dashboard/dist/chromenews_customizer_scripts.build.js',
    ...
    wp_localize_script(
      ...
      array(
        ...
        'strings' => array(
          'addElement' =>  esc_html__("Add Element", "chromenews"),
          'removeElement' =>  esc_html__("Remove Element", 'chromenews'),
          'configureElement' =>  esc_html__("Configure Element", "chromenews"),
          'desktop' =>  esc_html__("Desktop", "chromenews"),
          'elementSettings' =>  esc_html__("Element Settings", "chromenews"),
        ),
      )
    );
  }
  ...
  public function enqueue_preview_scripts()
  {
    ...
    wp_enqueue_script(
      ...
      array('jquery', 'customize-preview', 'chromenews-fixed-header-script'),
      '1.0.0',
      true
    );
  }
